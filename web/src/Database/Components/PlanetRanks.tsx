...
const PlanetsRank = ({ ...props }: Props) => {
    ...
    const items = React.useMemo(() => {
        const ranks: Rank[] = [
            { name: 'latest', getter: p => p.distance, format: v => !v ? '' : Units.format(v, UnitType.LY), title: strings.distance, formatValue: v => !v ? '' : Dates.formatDate(v), titleValue: strings.discoveryDate },
            { name: 'nearest', getter: p => Value.Planet.prop(p, 'diameter'), format: v => !v ? '' : Units.format(v, UnitType.EARTH_RAD), title: strings.diameter, formatValue: v => !v ? '' : Units.format(v, UnitType.LY), titleValue: strings.distance },
            { name: 'similar', getter: p => Value.Planet.prop(p, 'diameter'), valueGetter: p => p.distance, format: v => !v ? '' : Units.format(v, UnitType.EARTH_RAD), title: strings.diameter, formatValue: v => !v ? '' : Units.format(v, UnitType.LY), titleValue: strings.distance },
        ]
            ...
            return {
                header: strings[rank.name], link: { pathname: Url.DATABASE }, content: (
                    <HierarchicalTable items={planets || []} columns={[
                        ...
                        { accessor: (planet: RankedPlanetData) => Value.Planet.props(planet, 'name'), title: strings.planet, render: (name, planet) => <IconText text={name} icon={`Database/Planet/${capitalCase(planet.properties[0].type || 'unknown')}.png`} size='2.35rem' />, width: 10 },
                        { accessor: (planet: RankedPlanetData) => rank.valueGetter?.(planet) || planet.value, render: rank.formatValue, title: rank.titleValue, width: '9rem' },

                ...
                levels: [
                    {
                        columns: Col.list<Star>([
                            ...
                            { name: 'name', format: (val, item) => <Detail pathname='/abc' title={item.properties[0].name} subtitle={<Colored color={spectralClassColor[item.properties[0].type.spectral_class || 'default']}>{strings.stars.colors[item.properties[0].type.spectral_class!]} {strings.stars.types[item.properties[0].type.size || 'unknown'].toLowerCase()} <SpectralType>{item.properties[0].type.spectral_class! + item.properties[0].type.spectral_subclass!}IIab</SpectralType></Colored>} />, width: 1.5, headerIcon: false },
                ...
                levels: [
                    {
                        columns: Col.list<Star>([
                            { name: 'type', format: (val, item) => <ItemImage image={`Database/Star/${item.properties[0].type.spectral_class}.svg`} large={true} />, width: '5rem', headerIcon: false },
                            { name: 'name', format: (val, item) => <Detail pathname='/abc' title={item.properties[0].name} subtitle={<Colored color={spectralClassColor[item.properties[0].type.spectral_class || 'default']}>{strings.stars.types[item.properties[0].type.size || 'unknown']} <SpectralType>{item.properties[0].type.spectral_class}</SpectralType></Colored>} />, width: 1.5, headerIcon: false },
                            { name: 'diameter', unit: '☉', multi: 'properties' },
                            { name: 'mass', unit: '☉', multi: 'properties' },
                            { name: 'density', unit: <Fraction top='kg' bottom={<>m<sup>3</sup></>}/>, multi: 'properties' },
                            { name: 'surface_temperature', unit: 'K', multi: 'properties' },

...
import { Dataset, Planet, Star } from '../types'
...
const SpectralClass = Styled.div`
...
const spectralClassColor: Record<string, string> = { [SpectralType.A]: '#DFF', [SpectralType.B]: '#3FF', [SpectralType.F]: '#EE0', [SpectralType.G]: '#CC0', [SpectralType.K]: '#F80', [SpectralType.M]: '#F50', [SpectralType.O]: '#0FF' }


// TODO: Hooks instaed providers?
// TODO: Root strings in parameter.
export const provideFilterColumns = (table: DbTable, strings: any): [string, string, { text: string, value: string }[]?][] => {
    switch (table) {
        case DbTable.BODIES:
            return []
        case DbTable.STARS:
        ...
        case DbTable.PLANETS:
            return [
                ['name', strings.properties.name],
                ['type', strings.properties.type, Object.values(PlanetType).map(value => ({ text: strings.planets.types[value], value }))],
                ['diameter', strings.properties.diameter + ' [⊕]'],
                ['mass', strings.properties.mass + ' [⊕]'],
                ['density', strings.properties.density + ' [kg/m^3]'],
                ['surface_temperature', strings.properties.surfaceTemperature + ' [°C]'],
                ['semi_major_axis', strings.properties.semiMajorAxis + ' [au]'],
                ['orbital_period', strings.properties.orbitalPeriod + ' [d]'],
                ['orbital_velocity', strings.properties.orbitalVelocity + ' [km/s]'],
                ...
                ['distance', strings.properties.distance + ' [ly]'],
                ['transit_depth', strings.properties.transit],
                ['datasets', strings.properties.dataset]
            ]
        case DbTable.DATASETS:
            return [
                ['name', strings.properties.name],
                ['type', strings.properties.type],
                ['total_size', strings.properties.objects],
                ['processed', strings.properties.processed],
                ['time', strings.properties.processTime],
                ['created', strings.properties.published],
                ['modified', strings.properties.lastActivity],
                ['priority', strings.properties.priority],
                ['url', strings.properties.url]
            ]
    }
                ...
                levels: [
                    {
                        columns: Col.list<Star>([
                            { name: 'type', format: (val, item) => <ItemImage image={`Database/Star/${item.properties[0].spectral_type![0]}.svg`} large={true} />, width: '5rem', headerIcon: false },
                            { name: 'name', format: (val, item) => <Detail pathname='/abc' title={item.properties[0].name} subtitle={<Colored color={spectralClassColor[item.properties[0].spectral_type![0]]}>{strings.stars.types[item.properties[0].type]} <SpectralClass>{item.properties[0].spectral_type}</SpectralClass></Colored>} />, width: 1.5, headerIcon: false },
...
// 622 601, 548, 601, 523, 463, 456, 367, 228
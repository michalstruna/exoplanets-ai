...
\lstinputlisting[caption={Spuštění serveru.}]{codes/run_server.txt}

\vspace{-5pt}

Sestavená webová aplikace je v~adresáři \code{web/build}. Lze ji umístit na  vlastní webový server s~přístupovým bodem \code{index.html} nebo tento soubor přímo otevřít v~prohlížeči.
...
\lstinputlisting[caption={Spuštění serveru s vlastní konfigurací.}]{codes/run_server_custom_config.txt}
...
Stejným způsobem můžeme upravit i~konfigurační soubor \code{client/constants/config.py} pro klientskou aplikaci.
...
\lstinputlisting[caption={Spuštění v~režimu pro vývoj.}]{codes/run_server_dev_config.txt}
...
\lstinputlisting[caption={Spuštění testů aplikace.}]{codes/test_server.txt}
...
\lstinputlisting[caption={Spuštění validace aplikace.}]{codes/validate.txt}

...
Vztah změny rychlosti hvězdy, hmotnosti hvězdy a~hmotnosti planety je dán vzorcem:

\formula[\cite{methods}]{Radiální rychlost}{K = \sqrt[3]{\frac{2\pi G}{T}} * \frac{M_p * sin(i)}{\sqrt[3]{(M_p + M_s)^2}} * \frac{1}{\sqrt{1 - e^2}}}{
\begin{tabular}{lll}
	K = amplituda změny rychlosti & $M_s$ = hmotnost hvězdy & $M_p$ = hmotnost planety \\
	sin(i) = sklon dráhy vůči pozorovateli & e = excentricita dráhy & T = oběžná doba \\
\end{tabular}
}

Z~radiální rychlosti lze vypočítat změnu vlnové délky záření:

\formula{Změna vlnové délky na základě rychlosti}{\Delta\lambda = \lambda_0 * \frac{v}{c}}{
\begin{tabular}{lll}
	$\Delta\lambda$ = změna vlnové délky & $\lambda_0$ = klidová vlnová délka & v = rychlost \
\end{tabular}
}

Dle něj lze vypočítat, jak některé planety ovlivňují radiální rychlost své hvězdy. Z~tabulky níže je patrné, 
...
\tab{Příklady ovlivňování radiální rychlosti hvězd planetami}{
	\begin{tabular}{|l|l|l|l|l|l|l|l|}
		\hline	
		\rowcolor{lightgray}
		Těleso & Hvězda & $M_p$ [kg] & $M_s$ [kg] & sin(i) & e & T [r] & \textbf{K [$\frac{m}{s}$]} \\	
		\hline	
		Země & \multirow{3}{*}{Slunce} & $5,97 * 10^{24}$ & \multirow{3}{*}{$1,99 * 10^{30}$} & 1 & 0,017 & 1 & \textbf{0,1} \\
		\cline{1-1}\cline{3-3}\cline{5-8}
		Jupiter & & $1,9 * 10^{27}$ & & 1 & 0,048 & 11,86 & \textbf{12,5} \\
		\cline{1-1}\cline{3-3}\cline{5-8}
		\cline{1-1}\cline{3-3}\cline{5-8}
		Pluto & & $1,9 * 10^{27}$ & & 1 & 0,048 & 11,86 & \textbf{12,5} \\
		\cline{1-1}\cline{3-3}\cline{5-8}
		\hline


		$\alpha$ Cen Bb & $\alpha$ Cen B & $6.75 * 10^{24}$ & $1.8 * 10^{30}$ & 1 & 0 & 0,0089 & \textbf{0,51} \\		
		\hline	
	\end{tabular}
}
}

Radiální rychlost známe právě díky metodě radiálních rychlostí. Z~rovnice lze vyjádřit a vypočítat hmotnost planety $M_p$. Pokud Země neleží přímo v rovině oběžné dráhy planety, kvůli $sin(i)$ nabízí metoda radiálních rychlost výpočet pouze minimální hmotnost planety.
...
\img[1]{Astrometrie}{img/astrometry.png}

...
Další z~důležitých charakteristik orbity je inklinace (sklon), která nám řekne, jaký úhel svírají roviny pozorovatele a~oběhu exoplanety. Bohužel lze zjistit minimální úhel sklonu dráhy a~nikoliv jeho přesnou hodnotu. Inkinace se bude zpravidla blížit 90°.~\cite{transitprops}
...
Jednou z~oblastí patřících do oboru umělé inteligence jsou umělé neuronové sítě, jež se inspirovaly biologickými neuronovými sítěmi. Jejich použití je vhodné na problémy, u~kterých neexistuje přesný matematický popis řešení nebo sice existuje, ale jeho realizace by byla příliš složitá. Hlavními oblastmi, kde se neuronové sítě využívají, jsou např.~\cite{nn}:
...
\item \textbf{Řízení v~dynamicky se měnících podmínkách} (autopilot, ...).
...
\tab[1]{Komponenty formálního neuronu}{
	\begin{tabularx}{\textwidth}{|X|X|X|X|}
		...
		\hline	
		Váhy spojení (vstupu) $w_{1...R}$ & Každý vstup má svou váhu. & [0.01, 0.56, 0.12] \\
		\hline	
...
\footnotetext[1]{Zdroj dat: \cite{nn}}
...
Každý neuron může mít jeden nebo více vstupů. Tyto vstupy jsou nejčastěji číselného charakteru a~mohou být dvojího druhu:
...
Spojení existují pouze mezi neurony sousedních vrstev. Ani neurony stejné vrstvy, ani neurony nesousedících vrstev nemohou být přímo propojeny. Orientace těchto spojů je navíc směrem pouze od vstupů k~výstupu sítě (proto dopředná)~--~výstupy neuronů v~předešlé vrstvě jsou vstupem do všech neuronů v~následující vrstvě.~\cite{nn}
...
Dále je třeba brát v~úvahu velikost obrázku. Full HD obrázek (1~920~*~1~080) se skládá z~více jak 2~milionů pixelů. V~případě RGB obrázku se množství informací zvýší až na 6~milionů (3~složky pro každý pixel). Posílat takové množství hodnot do FFNN, která se skládá z~plně prokojených vrstev, kde jsou neurony v sousedních vrstvách propojeny způsobem „každý z~každým“, by bylo problematické. Z~tohoto důvodu existuje tzv. pooling vrstva popsaná v~kapitole 4.3.3, která umožňuje zmenšit velikost vstupu a~přitom zachovat vzory charakteristické pro vstup.
...
\footnotetext[1]{Zdroj dat: \cite{nn}} % TODO: Keras API.
...
\formula{Provádění konvoluce pro výřez vstupu a~filtr}{y = \sum_{i=1}^{sy}{\sum_{j=1}^{sx}{v_{ij}f_{ij}}}}{
\begin{tabular}{cccc}
	y = výstup & $sx, sy$ = horizontální/vertikální velikost filtru & v = výřez vstupu & f = filtr\\
\end{tabular}
}

Výřez se postupně posouvá (\textit{stride}) v~rámci vstupu a~pro každou kombinaci výřezu a~daného filtru se výsledek uloží do výstupní struktury na odpovídající místo vzhledem ke vstupu. Výsledkem je tak struktura (\textit{feature map}) stejně velká nebo menší, než vstup~--~v~závislosti na vyplnění/oříznutí okrajů (\textit{padding}).
...
\footnotetext[2]{Zdroj dat:~\cite{keras}}
...
Obdobně k~vrstvě Max-Pooling existuje např. i~Min-Pooling nebo Average-Pooling. Jejich fungování je stejné s~tím rozdílem, že na výstup předávají místo maximální hodnoty tu minimální nebo průměrnou. Jejich použití je opět závislé na typu řešeného problému (např. zda vstupní obrázek ma světlé pozadí a~tmavé popředí či naopak).~\cite{keras}

\subsubsection{Vrstva Flatten}

Konvoluční vrstvy umí pracovat s~vícerozměrnými vstupy, vrstvy typu FC však očekávají pouze jednorozměrný vektor vstupních hodnot. Protože např. v~konvolučních sítích je třeba oba typy vrstev kombinovat, je nutné nějakým způsobem převést n-rozměrnou strukturu na vektor. Právě k~tomu slouží vrstva Flatten.~\cite{keras}
...
\footnotetext[2]{Zdroj dat: \cite{nn}} % TODO: Keras API.

\subsection{Učení neuronové sítě}

Po zapojení neuronů je třeba neuronovou síť naučit řešit daný problém. Učení může být dvojího druhu:

\begin{itemize}
\item \textbf{S učitelem}~--~Neuronové síti je předložen vstup z~tzv. trénovací množiny a~ta pro tento vstup vyvodí výstup, který je porovnán s~požadovaným výstupem. Na základě chyby mezi skutečným a~požadovaným výstupem pak síť za využití tzv. učícího algoritmu upraví váhy mezi neurony tak, aby se chyba minimalizovala. Tento postup je opakován pro všechny dvojice \textit{vstup $\Rightarrow$ výstup} v~trénovací množině~--~této iteraci se říká \textit{epocha}. Učení sestává z~jedné nebo více epoch a~může skončit:
